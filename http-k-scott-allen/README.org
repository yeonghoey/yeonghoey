...
most invalid requests *must be handled by providing some pages like 200 OK* for the user experiences.

In web services which serve data like ~json~ or ~xml~, on the other hand,
invalid requests *must be handled by HTTP Status Codes*,
