...
- [[#how-to][How-to]]
  ...
  - [[#get-a-reference-to-the-current-module][Get a reference to the current module?]]
...
*** KeyboardInterrupt
- Normally, ~KeyboardInterrupt~ is raised by the python defined ~default_int_handler~ for ~SIGINT~ ([[https://hg.python.org/cpython/file/6eefdf0fade6/Modules/signalmodule.c#l603][code]])
- However, if signal mask contains ~SIG_IGN~, it won't, which means, There would be no ~KeyboardInterrupt~.
- Most shell like ~bash~ set the signal mask of ~SIG_IGN~ to ~SIGINT~ and ~SIGQUIT~ when it runs the process background (like ~$ cmd &~)
-----
...
- [[https://docs.python.org/3/reference/import.html#module-path][__path__]] ::
  - only for a package.
  - a list of the package directory path string
  - used during imports of its subpackages

- [[https://docs.python.org/3/reference/import.html#__package__][__package__]] ::
  if the module is a package, ~__package__~ is equal to ~__name__~
  if not, it should be set to its parent package or empty when it's a top-level module
  This value is used to calculate explicit relative imports
...
#+END_SRC
-----
- https://stackoverflow.com/questions/1676835/python-how-do-i-get-a-reference-to-a-module-inside-the-module-itself
